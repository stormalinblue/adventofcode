...
impl Crosses for (&ClopenSegment, &ClopenSegment) {
    fn crosses(&self) -> bool {
        let (seg1, seg2) = *self;

        if seg1.points.first == seg2.points.first {
            return true;
        }

        if seg1.points.first != seg2.points.second
            && is_between_incl(&seg1.points.first, &seg2.points.first, &seg2.points.second)
        {
            return true;
        }

        if seg2.points.first != seg1.points.second
            && is_between_incl(&seg2.points.first, &seg1.points.first, &seg1.points.second)
        {
            return true;
        }

        let open_1 = OpenSegment {
            points: seg1.points,
        };
        let open_2 = OpenSegment {
            points: seg2.points,
        };

        if (&open_1, &open_2).crosses() {
            return true;
        }
